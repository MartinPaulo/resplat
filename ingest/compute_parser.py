...
from ingest.utils import get_product_dict, get_collection_appl_id_map, \
    parse_float, DataSizes
from storage.models import Ingest


class UOMComputeParser(AbstractBaseVicNodeUOMParser):
    COL_SIZE = 7

    def __init__(self, extraction_date, user):
        super(UOMComputeParser, self).__init__(extraction_date, user,
                                               get_collection_appl_id_map(),
                                               get_product_dict(),
                                               self.COL_SIZE)

    def get_start_after_line(self):
        return 'tenant,vicnode_id,tenant_id,vault_quota,' \
               'vault_used,compute_quota,compute_used'

    def get_storage_product_key_for_parser(self, cols):
        return 'Computational.Melbourne'

    def get_allocated_capacity_column_details(self):
        return 6, DataSizes.GIGABYTE.bit_conversion_factor_gig

    def get_used_capacity_column_details(self):
        return 7, DataSizes.GIGABYTE.bit_conversion_factor_gig

    def process_ingest_row_data(self, extracted_ingest):
        if not isinstance(extracted_ingest, Ingest):
            return None, 'Expected Instance object, got ' + type(
                extracted_ingest)
        if extracted_ingest.allocated_capacity == 0 and \
                extracted_ingest.used_capacity == 0:
            return (True,
                    'Zero values for allocated and used capacity, '
                    'ingest ignored')
        try:
            Ingest.objects.get(
...
class DummyParserWithoutSave(UOMComputeParser):
    def process_ingest_row_data(self, extracted_ingest):
        return False, 'No save done'


class ParseUOMComputeWithoutSaveTestCase(IngestTestCase):
    def setUp(self):
    ...
    def testValidParse(self):
        lineStream = [
            'tenant, vicnode_id, tenant_id, vault_quota, vault_used, '
            'compute_quota, compute_used',
            'Monash_Forensic_Data, 2013R4.1, '
            '5df4e9199a914449a14f88d22b53e63b, 0.0, 0.0, 0.0, 0.0',
            'C4D_Renderfarm, 2014R9.05, '
            '57fb4d3f73534337b94fe5bbb8d04849, 0.0, 0.0, 100.0, 100.0',
            'Epoch_of_Reionization, 2014R5.1, '
            'a75f28a8621344f187d5ccecf171649f, 0.0, 0.0, 0.0, 0.0',
            'AURIN-workflow, 2013R1.1, '
            'ed15a17952044ee4b2a260af81daa3f5, 465.661, 0.0, 4500.0, 4500.0',
            'nex_gen_seq, 2014R6.2, '
            '51c417fd7c7b4d648f58684065789682, 51200.0, 0.0, 3000.0, 3000.0',
            'Interferome, 2014R9.06, '
            '96a85d08c50945f0b95c0691ab8c1094, 0.0, 0.0, 100.0, 100.0',
            'VicRoads_open_data, 2014R14.5, '
            '5efcf00a5431448586564b8341ba6a17, 10240.0, 0.0, 200.0, 200.0',
            ...
            'ACFS, 2014R7.1, '
            'c2c73c84d8174a34a8794b7ab199eed1, 0.0, 0.0, 100.0, 100.0',
            'BIOENG_N67, 2014R8.2, '
            '0608c0d6dbfc4980b703f86f987bc81b, 0.0, 0.0, 15360.0, 15360.0',
            'OzFlux_Data_Portal, 2014R5.2, '
            '7a8085ba38d548249163a368f5772a9b, 0.0, 0.0, 160.0, 160.0',
            'IMOS, 2013R1.4, '
            'c76b369565e843b28a4338491001d15b, 0.0, 0.0, 0.0, 0.0',
            ...
            '1kp_services, 2014R6.4, '
            '6119cd97a3bd478d809845aeacc6ea12, 0.0, 0.0, 0.0, 0.0',
            'Geodataserver, 2014R8.3, '
            '09a278ca47724f958760634ea333e25b, 0.0, 0.0, 20580.0, 20580.0',
            'RemoteLab_VeRSI, 2013R4.4, '
            '8021e6d7ac9a4786b9db0a2e907495eb, 0.0, 0.0, 0.0, 0.0',
            'Florey_Neuroscience_Collection, 2014R10.3, '
            '0437f08e793243de9ad938f55ec014c4, 29802.322, 0.0, 0.0, 0.0',
            'CO2CRC_Extranet, 2014R11.02, '
            '2d64ad51a8d94ce29ef6e14adfb9e60a, 0.0, 0.0, 50.0, 50.0',
            'Endo_VL, 2014R8.6, '
            '2bb45090fdb74f8c81359fbe723e4ab5, 0.0, 0.0, 20000.0, 20000.0',
        ...
        for line in lineStream:
            ...
            if count > 1:
                cols = line.split(',')
                parse_val, parse_err = self.parser.parse_extraction_date(cols)
                check_val, check_err = self.extractDate, None
                self.check(parse_val, parse_err, check_val, check_err,
                           'extractDate', count)

                parse_val, parse_err, colNum = \
                    self.parser.parse_collection_object(cols)
                check_val = self.collectionDict.get(
                    cols[1])  # get() will return None instead of keyError
                if check_val:
                ...
                prodKey = 'Computational.Melbourne'
                ...
                parse_val, parse_err = \
                    self.parser.parse_allocated_capacity_in_gb(cols)
                check_val, check_err = parse_float(cols[5])
                check_val *= 1000
                ...
                check_val, check_err = parse_float(cols[6])
                check_val *= 1000
                if parse_err and parse_err.startswith('internal'):
                    check_err = 'Internal error: ' \
